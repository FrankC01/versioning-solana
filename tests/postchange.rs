use common::{
    get_account_for_key, get_keypair, set_string_value, set_u64_value, setup_validator, PROG_KEY,
    USER1_ACCOUNT, USER2_ACCOUNT,
};
use solana_sdk::{account::ReadableAccount, commitment_config::CommitmentConfig, signer::Signer};

mod common;

#[test]
fn test_pre_data_change_load_pass() {
    let (test_validator, _initial_keypair) = setup_validator().start();
